...
function wp_bootstrap_starter_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'business-labor-theme' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'business-labor-theme' ),
    ...
    if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
        ...
        comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'business-labor-theme' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
        echo '</span>';
    }
...
function wp_bootstrap_starter_entry_footer() {
	...
	edit_post_link(
		sprintf(
			...
			esc_html__( 'Edit %s', 'business-labor-theme' ),
       ...
       // $GLOBALS['comment'] = $comment;
        ...
        if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) : ?>
            ...
            <div class="comment-body">
                <?php _e( 'Pingback:', 'business-labor-theme' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'business-labor-theme' ), '<span class="edit-link">', '</span>' ); ?>
            </div>
                ...
                <div class="media-body">
                    <div class="media-body-wrap card">
                        ...
                        <div class="card-header">
                            <h5 class="mt-0"><?php printf( __( '%s <span class="says">says:</span>', 'business-labor-theme' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h5>
                            <div class="comment-meta">
                                <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                                    <time datetime="<?php comment_time( 'c' ); ?>">
                                        <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'business-labor-theme' ), get_comment_date(), get_comment_time() ); ?>
                                    </time>
                                </a>
                                <?php edit_comment_link( __( '<span style="margin-left: 5px;" class="glyphicon glyphicon-edit"></span> Edit', 'business-labor-theme' ), '<span class="edit-link">', '</span>' ); ?>
                            </div>
                        </div>

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'business-labor-theme' ); ?></p>
                        <?php endif; ?>
